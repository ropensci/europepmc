...
Be careful with the interpretation of the slower growth in the last years because Europe PMC also contains journals, which make their articles openly available after an embargo period that can be up to three years.

To assess journal participation in Europe PMC and when Europe PMC journals make their content freely available, we can simply parse [Europe PMC's Journal list](https://europepmc.org/journalList)

```{r, results='asis'}
library(dplyr)
u <- "https://europepmc.org/journalList?format=csv"
jn_access_lvl <- httr::GET(u) %>%
  httr::content("text/csv", as = "parsed", encoding = "UTF-8") %>%
  dplyr::count(`Free access`, sort = TRUE) %>% 
  dplyr::mutate(proportion = n / sum(n))

jn_access_lvl %>%
  knitr::kable(caption = "How soon after publication do participating journals make their articles freely available in Europe PMC?")
```

The table reveals that `r round(jn_access_lvl[1,2] / sum(jn_access_lvl$n) * 100)`% of all journals make their content immediately available via Europe PMC.
...
ggplot(my_df, aes(factor(year), query_hits / refs_hits, group = query_id, 
  ...
  xlab("Year published") +
  ylab("Proportion of article in Europepe PMC")
```
