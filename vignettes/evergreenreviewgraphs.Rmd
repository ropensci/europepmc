...
Trend graphs in literature reviews illustrate the development of concepts in scholarly communication. Some trend graphs, however, don't acknowledge that the number of scholarly publications is growing each year, but simply display the absolute number of hits they have found for a given concept. According to Noam Ross, these misleading graphs are called evergreen review graphs, and examples can be found on Twitter under the hash-tag [#evergreenreviewgraph](https://twitter.com/hashtag/evergreenreviewgraph). 

This vignette guides you how to make proper review graphs. In these graphs, the number of hits found is divided by the total number of records indexed in Europe PMC for a given search query. 
...
Be careful with the interpretation of the slower growth in the last years because Europe PMC also contains journals, which make their articles openly available after an embargo period that can be up to two years. 
...
Another problem of our query approach is that we did consider that DOIs can also be used to cite software, a best-practice implemented by [Zenodo and GitHub](https://guides.github.com/activities/citable-code/) or the [The Journal of Open Source Software](http://joss.theoj.org/). 

Lastly, it actually remains unclear, which and what kind of software is cited how often. We could not control if authors just cited the homepages and not a particular source code repository. One paper can also cite more than one code repository, which is also not represented in review graph. 
